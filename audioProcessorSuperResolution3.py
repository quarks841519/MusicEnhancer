...
def test_audiosr_super_resolution(input_audio_path, output_audio_dir, log_callback=None, completion_callback=None):
    ...
    try:
        ...
        for i in range(num_chunks):
            ...
            chunk_filename = f"chunk_{i:03d}.wav" # Still save as WAV for audiosr to load
        ...
        # --- CHANGE START ---
        output_audio_path = os.path.join(output_audio_dir, f"{name}_upscaled.flac") # Changed extension to .flac
        # --- CHANGE END ---

        log(f"Saving final upsampled audio to '{output_audio_path}' (48kHz)...")
        # --- CHANGE START ---
        torchaudio.save(output_audio_path, final_upsampled_waveform.cpu(), 48000, format="flac") # Specify format="flac"
        # --- CHANGE END ---
